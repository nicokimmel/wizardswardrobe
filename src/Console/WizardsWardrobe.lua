...
function WW.LoadSetup( zone, pageId, index, auto, skipValidation )
	...
	WW.equipped.zone = WW.selection.zone.tag
	WW.equipped.page = WW.selection.pageId
...
function WW.DeleteSetup( zone, pageId, index )
	...
	if WW.setups[ zone.tag ]
		...
		WW.Log( "Deleted Setup: ", WW.LOGTYPES.NORMAL, "FFFFFF", setupName )
	end
...
local function moveItemToDestination( item )
	...
	if item.destBag == BAG_WORN then
		RequestEquipItem( item.sourceBag, item.sourceSlot, BAG_WORN, item.destSlot )
	else
